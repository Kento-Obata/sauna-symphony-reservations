...
const ReservationForm = () => {
  ...
  const getTimeSlotReservations = (selectedDate: Date) => {
    ...
    if (!reservations || !selectedDate || !isValid(selectedDate)) {
      console.log("Invalid date or no reservations:", {
        selectedDate,
        reservations
      });
    ...
    const slotReservations = reservations.filter(r => r.date === dateString && (r.status === "confirmed" || r.status === "pending")).reduce((acc, r) => {
  ...
  return <div className="glass-card p-4 sm:p-8 animate-fade-in w-full max-w-4xl mx-auto">
      ...
      <form onSubmit={handleFormSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <ReservationDetails timeSlot={timeSlot} setTimeSlot={setTimeSlot} name={name} setName={setName} email={email} setEmail={setEmail} phone={phone} setPhone={setPhone} people={people} setPeople={setPeople} temperature={temperature} setTemperature={setTemperature} date={date} setDate={handleDateChange} timeSlotReservations={timeSlotReservations} />
        </div>
        ...
        <div className="text-center mt-8">
          <div className="mb-4">
            ...
            <p className="text-2xl font-bold text-stone-600">{formatPrice(totalPrice)}</p>
            {temperature && parseInt(temperature) <= 7 && <p className="text-sm text-muted-foreground mt-1">
      ...
      <TermsDialog open={showTermsDialog} onOpenChange={setShowTermsDialog} checked={termsAccepted} onCheckedChange={setTermsAccepted} />

      {currentReservation && <ReservationConfirmDialog isOpen={showConfirmDialog} onClose={() => setShowConfirmDialog(false)} onConfirm={handleConfirmReservation} reservation={currentReservation} onEdit={() => setShowConfirmDialog(false)} isSubmitting={isSubmitting} reservationCode={reservationCode} />}
    </div>;
};
